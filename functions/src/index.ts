...
async function ensureStableAuth(reportDoc: any) {
  const updates: any = {};
  if (!reportDoc.token) {
...
async function generatePdf(report: any, verifyUrl: string) {
  ...
  const params: any[] = Array.isArray(report.parameters) ? report.parameters : [];
  ...
  try {
    ...
    logger.warn("QR generation failed", e as any);
  }
...
export const report = onRequest({ cors: true, region: "asia-south1" }, async (req: any, res: any) => {
  try {
    ...
    const token = (url.searchParams.get("token") || (req.query?.token as string | undefined)) as string | undefined;
